...
double sick_1[max_step + 1], sick_2[max_step + 1], sick_3[max_step + 1],
    sick_4[max_step + 1];

double sick_all[max_step + 1], sick_wall[max_step + 1];
...
long double adaption_mode_fuzzy_real[_mem_length * _max_rule + 1];
double min_m[_in_varl + 1], max_m[_in_varl + 1];  // 最小中心點 & 最大中心點

double adaption_mode_min_m[_in_varl + 1], adaption_mode_max_m[_in_varl + 1];  // 最小中心點 & 最大中心點
...
double amcl_orientation_x, amcl_orientation_y, amcl_orientation_z,
    amcl_position_x, amcl_position_y, amcl_position_z;
double v1;
double v2;
...
int too_close_left = 0;
int too_close_right = 0;
...
void C_Rule::In_mem_fir(double *in, int _rule) {
  ...
  for (i = 1; i <= _in_varl; i++) {
    ...
    if (!change){
      ...
      in_mu =in_mu * phi(in[i], adaption_mode_fuzzy_real[(_rule - 1) * _mem_length + i * 2 - 1],adaption_mode_fuzzy_real[(_rule - 1) * _mem_length + i * 2]);
      //( 輸入  ，中心點                                   ,寬 )
    }

    // }
    // in_mu = in_mu * phi(in[i] , fuzzy_real[(_rule-1)*_mem_length+i*2-1] ,
    // fuzzy_real[(_rule-1)*_mem_length+i*2] ) ;//20211215
  }
...
*/
    ...
    left_min = minimun(180 * 2, 270 * 2, k);
    right_min = minimun(90 * 2, 180 * 2, k);
    stright_min = minimun(165 * 2, 195 * 2, k);
    left_colse = minimun(180 * 2, 240 * 2, k);
    right_colse = minimun(120 * 2, 180 * 2, k);
  }
  ...
  void decision_(int j) {
    ...
    if (decision_left == 1) {
      ...
      for (size_t i = 359; i < 403; i++) {
        if (read_4 > laser_temp[i]) read_4 = std::min(laser_temp[i], far);
      }
      // printf("%d\n",1);
      // 201.5~224
      for (size_t i = 403; i < 224 * 2; i++) {
        if (read_3 > laser_temp[i]) read_3 = std::min(laser_temp[i], far);
      }
      // printf("%d\n",2);
      // 224~246
      for (size_t i = 224 * 2; i < 246 * 2; i++) {
        if (read_2 > laser_temp[i]) read_2 = std::min(laser_temp[i], far);
      }
      // printf("%d\n",3);
      // 246~268.5
      for (size_t i = 246 * 2; i < 537; i++) {
        if (read_1 > laser_temp[i]) read_1 = std::min(laser_temp[i], far);
      }
      // printf("%d\n",4);
      // 90~270
      for (size_t i = 90 * 2; i <= 270 * 2; i++) {
        if (read_5 > laser_temp[i]) read_5 = std::min(laser_temp[i], far);
      }
      // printf("%d\n",5);
      // 90~180
      for (size_t i = 90 * 2; i <= 180 * 2; i++) {
        if (read_6 > laser_temp[i]) read_6 = std::min(laser_temp[i], far);
      }
  ...
  ///////////////////////////////  get distance value from different angles from
  ///sensor  ///////////////////////
  ...
  void test_open() {
    for (int ssss = 1; ssss <= _in_varl; ssss++) {
      min_m[ssss] =
          1.0e10;  // 為了找最小值，所以初始的最小值令為1，即為"最大"的最小值。
      max_m[ssss] =
          1.0e-10;  // 為了找最大值，所以初始的最大值令為0，即為"最小"的最大值。
      ////-1~1之間的中心點範圍
      // max_m[ber][ssss]=0.0;
    ...
    FILE *fnoise1, *fnoise0, *fnoise2, *fnoise3;
    printf("read file\n");
    if ((fnoise0 = fopen(normal_mode_clu, "r")) == NULL) {
      printf("Nothing1\n");
      exit(1);
    }
    ...
    if ((fnoise1 = fopen(normal_mode_FC, "r")) == NULL) {
      printf("Nothing2\n");
      exit(1);
    }
    ...
    if ((fnoise2 = fopen(adaption_mode_clu, "r")) == NULL) {
      printf("Nothing3\n");
      exit(1);
    }
    fscanf(fnoise2, "%d", &adaption_mode_in_clu);
    fclose(fnoise2);

    if ((fnoise3 = fopen(adaption_mode_FC, "r")) == NULL) {
      printf("Nothing4\n");
      exit(1);
    }
    for (int i = 1; i <= _mem_length * adaption_mode_in_clu; i++) {
      fscanf(fnoise3, "%Lf \n", &adaption_mode_fuzzy_real[i]);
      // printf("check%Lf\t",fuzzy_real[i]);
    }
    ...
    for (int jj = 1; jj <= adaption_mode_in_clu; jj++) {
      for (int jjj = 1; jjj <= _in_varl; jjj++) {
...
/*     printf(" ~~~~~~~~~~~~~~~~~~~~~~~~~~ \n");
  ...
  void Fir_str(double *in, int _rule, int ber) {
    ...
    if (!change){
      ...
      for (k = 1; k <= _rule; k++) {
        for (j = 1; j <= _out_varl; j++) {
          _Rule[k].con[j] = adaption_mode_fuzzy_real[k * _mem_length - _out_varl + j];
        }  // fuzzy_real為模糊控制器參數，這裡在讀取後鑑部的部分
        _Rule[k].In_mem_fir(in, k);  // 計算前鑑部激發量
      }
    }
  ...
  /////////////////////////////////////////
  void speed_limit() {
    if (out_y[1] >= max_speed) out_y[1] = max_speed;
    if (out_y[2] >= max_speed) out_y[2] = max_speed;
    if (out_y[1] <= -(max_speed)) out_y[1] = -max_speed;
    if (out_y[2] <= -(max_speed)) out_y[2] = -max_speed;
  }
  ////////////////////////////////////// speed limit
