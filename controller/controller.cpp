...
#define load_data_clu "controller/rule_s.txt"	//紀錄rule數量的文件

#define load_data_FC "controller/s.txt" 	//紀錄FC參數的文件
...
int main(int argc,char ** argv)
{
	...
	while(counts<max_step && rclcpp::ok())
	{
		counts++;
		
		printf("counts:%d\n",counts);
		node->test_open(); //to get max and min
		node->decision_(counts);
		node->get_sensor(counts) ;
		node->sick_limit(counts) ;//20200505
		node->fuzzy_in(counts) ;					//輸入是雷測數值，只是調整fuzzy的input比例			
		node->Fir_str(in , _in_clu , ber) ;	//讀取後件部計算機發量
		node->fuzzy(_in_clu , ber);  		//解模糊產生out_y
		//node->publisher_info(counts);
		node->test_run(counts);  //to do work
