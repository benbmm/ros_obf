...
#define _Maxstep 40200 //20220328
#define _count 1000
//-------------------------
...
//#define _rule   16          // 10 rules //20220330
...
// width = 0.05~0.3
#define _range_max  0.3 //20220328
#define _range_min  0.05
#define   wwidth     0.05  //20220330
#define over_par 0.5   //20220330
...
/// For learning
//#define _lpara  ((2*_in_varl+_out_varl)*_rule)  // 13*10 = 130
#define _lpara  ((2*_in_varl+_out_varl))  //20220330

#define _popsize    40         // a swarm contains 40 particles(粒子)
#define _maxgen 200        // 100 iteration //20220411
#define MAX_STEP    20000  // command amount
...
#define   garma03    1.0
#define new_row 20 
#define _nobj   3                   // the number of objects is 2
#define _mem_length  ((2*_in_varl)+_out_varl)
#define _max_rule 30  //20220407

//int _in_clu; //20220330
int best_solution[_nobj+1]={0}; //儲存最佳解的變數名稱
double costvalue_interpretable; //可解釋性的costvalue的暫存變數
double epsilon_sum[_popsize+1];
double penality[_popsize+1];  
double fuzzy_real_compare[_mem_length*_max_rule+1] ; //儲存"已排除重複規則"的暫存空間
void check_and_combine_child_set(int, int, int);  //fuzzy set的合併 //20220406
void overlap_degree(int,int,int);  //重疊度的計算
void fuzzy_set_constraint(int,int,int);  //fuzzy set之間的距離限制
//-------------interpretable------------
...
int save_gen(int);
void load_gen_data();
void save_final_obj(int); //20220414
void save_final_pareto_front();//20220414
void report_best() ; //回傳最佳解//20220414
...
public:
	...
	double p[_lpara*_max_rule+1], fitness[_nobj+1], CT[_nobj+1]; //_lpara=110 ,_nobj=2(目標個數) //20220330
...
double reference_FCRule[_lpara*_max_rule+1];
int ref_rule = 0;
// Fuzzy controller
...
public:
	...
	double FCRule[_lpara*_max_rule+1], FCS[_max_rule+1];  //FCRule在存110個參數，FCS在存10條rule的激發量//20220330
	int cal_rule; //20220330 為了算不同控制器模糊解模糊用到的rule
	void getRule(double*, int );                    // get parameter//20220330
          	void Controller(double*, double* );        // main FC
	void Fuzzifier(double* );	        // fuzzifier
	void Defuzzifier(double* );	        // defuzzifier
	void  FC_Str_Learn(double *,int, int*) ; //架構學習 ///20220330
	//void fuzzy_cluster(); //20220330
	void FC_In_mem(double *,int); //20220330
	void check_and_merge_in_train(int);
};

void FCR :: getRule(double *p, int rule){  //把每隻鳥(控制器)的110個參數傳給模糊控制內//20220330
    // read parameters from the input array
	for(int i=1; i<=_lpara*rule; i++){  //20220330
	  FCRule[i]=p[i];
	}
}
...
void FCR :: Fuzzifier(double *input){
    ...
    for(int i=1; i<=_in_varl; i++){ 
        ...
        for(int j=1; j<=cal_rule; j++){  //_rule=10
    ...
    for(int i=1; i<=cal_rule; i++){ //這邊開始是每條rule個別去看了，因為要去算mu值和激發量，(做10次)
            ...
            // for each input
    	        ...
    	        //這邊是算4個input進去得出的mu值連乘4次得到激發量
                   ...
                   // else: u(x) = gaussian membership function
                                 ...
                                 k*=Gaussian(input[j], FCRule[((_gap)*(i-1))+(2*(j-1))+1], FCRule[((_gap)*(i-1))+(2*(j-1))+2]);}     */ 
					k*=Gaussian(input[j], FCRule[((_gap)*(i-1))+(2*(j-1))+1], FCRule[((_gap)*(i-1))+(2*(j-1))+2]);    //20220328   
...
void FCR :: Defuzzifier(double* output){
	...
	for(int i=1; i<=_out_varl; i++){ //在求後件部，所以迴圈3次即可
		...
		// for each rule
		for(int j=1; j<=cal_rule; j++){ //共10條rule所以迴圈要10次
		...
		//output[i]=(fabs(den[i]) < 1E-8)?0:(num[i]/den[i]);
		output[i]=(fabs(den[i]) == 0)?0:(num[i]/den[i]); //20220328
	}          //3個後件部書出值output被算出來，如果分母太小輸出就強迫=0，如果正常就用Zmom解模糊
	//printf("%f %f \n",output[0],output[1]);
}

/////////////////////////  架構學習  ////////////////////////////
//以激發量為標準來決定是否應該產生一個新的模糊規則  

void FCR :: FC_Str_Learn(double *A_in, int c_gen, int* _in_clu )      /// FC架構學習 ///20220330
{
	int max_in=1, i,j,jjj;
	int temp_rule = 1;
	double min_d=100;
	...
	if  ( ! *_in_clu )    // initial mapping , 規則數是0的時候會進if 
	{
		*_in_clu = 1 ; //20220330
		/*for(i=1;i<=_in_varl;i++) 
		{ 
			_in_v[i].FC_parti_num = _in_v[i].FC_parti_num + 1 ;
			_Rule[1].FC_pre[i] = 1 ;
		}*/

		for (i=1; i<=_in_varl; i++)
		{
			//_in_v[i].mean[1] = A_in[i] ; ///202104
			//_in_v[i].var[1] = wwidth ;
			reference_FCRule[(*_in_clu-1)*_lpara+2*i-1] = A_in[i] ;
			reference_FCRule[(*_in_clu-1)*_lpara+2*i] = wwidth ;
			FCRule[(*_in_clu-1)*_lpara+2*i-1] = A_in[i]+randn(-0.1,0.1) ;
			FCRule[(*_in_clu-1)*_lpara+2*i] = wwidth+randn(-0.1,0.1) ;		
		}
		//fuzzy_cluster(1,ber,c_gen); ///20220330
		reference_FCRule[*_in_clu*_lpara-_out_varl+1] = 0;
		reference_FCRule[*_in_clu*_lpara-_out_varl+2] = 0;
		FCRule[*_in_clu*_lpara-_out_varl+1] = _uscale*((rand()%2001)-1000)/1000.;
		FCRule[*_in_clu*_lpara-_out_varl+2] = _uscale*((rand()%2001)-1000)/1000.;
	}
	else
	{
		//對於輸入數據，通過高斯隸屬函數在規則中找到最大激發量

		for(j=1;j<=*_in_clu;j++)      
		{
			FC_In_mem(A_in,j) ;    // 逐條計算每個"規則數"對應的激發量  

			if (FCS[j] > FCS[max_in])   
			{max_in = j ; } //紀錄最大激發量的"規則號碼"
		}
 
		//cout<<"  _Rule["<<max_in<<"].in_mu ="<<_Rule[max_in].in_mu;
		//若門檻值(_thre_in)比驗證用的激發量還大，即激發量小於閥值時，則"產生新的規則"。

		if ( FCS[max_in] < 0.00001)   ///20220422 0.01=>0.001
		{
			if(*_in_clu < _max_rule){///202105 新增一條FC規則
				*_in_clu = *_in_clu + 1 ; 
				/*float sum =0;
				for(i=1;i<=_in_varl;i++){///202104
					sum = sum +  pow ( A_in[i] - FCRule[(max_in-1)*_lpara+2*i-1],2) ;///202104
				}*/

				for(i=1;i<=_in_varl;i++){
					//_in_v[i].FC_Add_cluster(A_in,i,_in_clu,sum) ;  //中心點、寬度的初始生成 
					reference_FCRule[(*_in_clu-1)*_lpara+2*i-1] = A_in[i] ;
					//FCRule[(*_in_clu-1)*_lpara+2*i-1] = A_in[i] ; 
					//FCRule[(*_in_clu-1)*_lpara+2*i] = over_par*sqrt(sum) ;	
					//FCRule[(*_in_clu-1)*_lpara+2*i] = wwidth;
					reference_FCRule[(*_in_clu-1)*_lpara+2*i] = wwidth ;
				}
				//fuzzy_cluster(1,ber,c_gen); ///20220330
				reference_FCRule[*_in_clu*_lpara-_out_varl+1] = 0;
				...
				for(jjj=1; jjj<=_in_varl; jjj++) //20221127
				{
					min_d = 100;
					for(i = 1; i<=*_in_clu-1; i++)
					{
						if(min_d > fabs(reference_FCRule[(*_in_clu-1)*_lpara+jjj*2-1]- reference_FCRule[(i-1)*_lpara+jjj*2-1]) ){
							min_d = fabs(reference_FCRule[(*_in_clu-1)*_lpara+jjj*2-1]- reference_FCRule[(i-1)*_lpara+jjj*2-1]);
							temp_rule = i;
						}
					}
					if((min_d - garma03*wwidth) < 0){
						reference_FCRule[(*_in_clu-1)*_lpara+jjj*2-1] = reference_FCRule[(temp_rule-1)*_lpara+jjj*2-1];
						reference_FCRule[(*_in_clu-1)*_lpara+jjj*2] = reference_FCRule[(temp_rule-1)*_lpara+jjj*2];
					}
					else{
						reference_FCRule[(*_in_clu-1)*_lpara+jjj*2] = min_d/garma03;
					}
					FCRule[(*_in_clu-1)*_lpara+2*i-1] = reference_FCRule[(*_in_clu-1)*_lpara+jjj*2-1]+randn(-0.1,0.1) ;
					FCRule[(*_in_clu-1)*_lpara+2*i] = reference_FCRule[(*_in_clu-1)*_lpara+jjj*2]+randn(-0.1,0.1) ;		
				}  
			}
			else{cout << "too many FC"<<endl ;}///限制在_max_rule條FC規則
		}	
	...
	{
		if (FCRule[(*_in_clu-1)*_lpara+jjj*2-1] < _center_min)
		{
			FCRule[(*_in_clu-1)*_lpara+jjj*2-1] = _center_min ; // 限制中心最小值
		}

		if (FCRule[(*_in_clu-1)*_lpara+jjj*2-1] > _center_max)
		{
			FCRule[(*_in_clu-1)*_lpara+jjj*2-1] = _center_max ; // 限制中心最大值
		}

		if (FCRule[(*_in_clu-1)*_lpara+jjj*2] < _range_min)
		{
			FCRule[(*_in_clu-1)*_lpara+jjj*2] = _range_min ; // 限制寬度最小值
		}

		if (FCRule[(*_in_clu-1)*_lpara+jjj*2] > _range_max)
		{
			FCRule[(*_in_clu-1)*_lpara+jjj*2] = _range_max ; // 限制寬度最大值
		}
	}	
...
///////////////////////  驗證"規則數"的激發量計算  ///////////////////
void FCR::FC_In_mem(double *B_in,int jj )  // 計算激發量  ///20220330
{
	int i ;
	...
	for(i=1;i<= _in_varl;i++)
	{ 
		//in_mu = in_mu * Gaussian(B_in[i],FCRule[(jj-1)*_lpara+2*i-1],FCRule[(jj-1)*_lpara+2*i]) ;
		in_mu = in_mu * Gaussian(B_in[i],reference_FCRule[(jj-1)*_lpara+2*i-1],reference_FCRule[(jj-1)*_lpara+2*i]) ;
	}
	FCS[jj] = in_mu;
	//讀進來的輸入項激發量的累乘會作為判斷依據。

}
///////////////////////  驗證"規則數"的激發量計算  ///////////////////

//////////////////////// 20220402邊控制編合併 ///////////////////////
void FCR::check_and_merge_in_train(int _in_clu)
{
 double check_dis_variable=0.0;
  double check_combine_data[3]={0.};
  double bigger_width=0.0;

  //中心點的轉存
  double child_sort_set_center[_in_varl+1][_max_rule+1]={0.0}; 
  double child_sort_set_center_backup[_in_varl+1][_max_rule+1]={0.0}; 
  
  //標準差的轉存
  double child_sort_set_width_backup[_in_varl+1][_max_rule+1]={0.0}; 

  ////////////// 檢查合併的暫存變數 //////////////

/////////////////////////////  原始資料的轉存  ////////////////////////////{
  //int bb=1;
  for(int a=1; a<=_in_clu; a++){  //jj表示第幾條rule 
    for(int b=1; b<=_in_varl; b++){ //jjj表示第幾個輸入
   
      child_sort_set_center[b][a] = FCRule[((a-1)*_mem_length)+((b*2)-1)] ; // 中心點
      //printf("child_sort_set_center[%d][%d]=%lf\n",b,a,child_sort_set_center[b][a]);
      child_sort_set_center_backup[b][a] = FCRule[(a-1)*_mem_length+b*2-1] ; // 中心點    
      child_sort_set_width_backup[b][a] = FCRule[(a-1)*_mem_length+b*2] ; // 寬度   
...
////////////////////////  利用插入排序法排列中心點的大小順序  ////////////////////////
  ...
  for(int jjj=1; jjj<=_in_varl; jjj++){
    int isolate_temp_var=0;
    double isolate_temp_save=0.0; 

    for(int kk=2;kk<=_in_clu;kk++){
      isolate_temp_var=1; 	 
      isolate_temp_save=child_sort_set_center[jjj][kk];
	 
      while (isolate_temp_save > child_sort_set_center[jjj][isolate_temp_var] && isolate_temp_var<kk ){	 
        isolate_temp_var++;	 
      }//if no2>no1 1<2,isolate_temp_var=2
	 
      if (isolate_temp_var!=kk){
        for (int i = kk; i > isolate_temp_var; i--){
          child_sort_set_center[jjj][i]=child_sort_set_center[jjj][i-1];		 
        }  		 
        child_sort_set_center[jjj][isolate_temp_var]=isolate_temp_save;	 
      }
    }
  }//中心點依照輸入做為區別，並由小到大排序

  int save_clu_num[_in_varl+1][_max_rule+1]={0};  //重新排序後的回到排序前
  int save_clu_num_inverse[_in_varl+1][_max_rule+1]={0}; 

  for(int jjj=1; jjj<=_in_varl; jjj++){
    for(int jj=1; jj<=_in_clu; jj++){
      for(int kk=1; kk<=_in_clu; kk++){
        if (child_sort_set_center_backup[jjj][jj]==child_sort_set_center[jjj][kk]){
          save_clu_num[jjj][jj]=kk;   //重新排序後的狀態回到排序前的狀態的轉換變數
          save_clu_num_inverse[jjj][kk]=jj;   // 未排序的狀態直接轉換到已經排序後的狀態的轉換變數
        }				
      }
    }
  }
  //中心點已完成排序後，為了找到相對應的寬度是第幾個
  //利用比對的方式，當原本的中心點時等於已完成由小到大排序後的中心點時，記錄為排序時第jj條規則在排序後位於第kk個
  //此時save_clu_num為10條規則已排序後的順序，save_clu_num_inverse為未排序前rule的順序
  //例如123在排序後為231
  //則save_clu_num[1]=3 save_clu_num_inverse[3]=1 
    //save_clu_num[2]=1 save_clu_num_inverse[1]=2
    //save_clu_num[3]=2 save_clu_num_inverse[2]=3
  double x=0.0;
  int left_n;
  for(int jj=1; jj<=_in_varl; jj++){
    for(int m=2;m<=_in_clu;m++){
      int count=0;
      for(int n=1;n<=m-1;n++){
        check_dis_variable = fabs(child_sort_set_center[jj][m]-child_sort_set_center[jj][n]);  //計算兩個set的中心點距離
		if(child_sort_set_center[jj][m] > child_sort_set_center[jj][n])
			left_n = n;
		else
			left_n = m;
        //已排序的中心點持續向前檢查，ex.2查1，3查1再查2....10查1~9
        ////////////////////  兩個set之間的寬度取大的當檢查標準  /////////////////
        if(child_sort_set_width_backup[jj][save_clu_num_inverse[jj][m]]>=child_sort_set_width_backup[jj][save_clu_num_inverse[jj][n]]){				
          bigger_width=child_sort_set_width_backup[jj][save_clu_num_inverse[jj][m]];			
        }
        else{				
          bigger_width=child_sort_set_width_backup[jj][save_clu_num_inverse[jj][n]];			
        }
        ////////////////////  兩個set之間的寬度取大的當檢查標準  /////////////////

        
       if(check_dis_variable<(garma02*bigger_width) || check_dis_variable < 0.05){//20220402如果滿足條件就合併，兩個中心點都變成一樣且留著        
          if(count==0){x=child_sort_set_center[jj][m];}
          count=1;
          //////////////////////  中心點  /////////////////////
          check_combine_data[0]=(x+child_sort_set_center[jj][n])*0.5;  //合併後的中心點數值
      	  //printf("child_sort_set_center[%d][%d]=%lf\tchild_sort_set_center[%d][%d]=%lf\n",jj,m,child_sort_set_center[jj][m],jj,n,child_sort_set_center[jj][n]);		
          child_sort_set_center[jj][m]=check_combine_data[0];//printf("child_sort_set_center[%d][%d]=%lf\t",jj,m,child_sort_set_center[jj][m]); //中心點數值存回去
          child_sort_set_center[jj][n]=check_combine_data[0];//printf("child_sort_set_center[%d][%d]=%lf\n",jj,n,child_sort_set_center[jj][n]); //中心點數值存回去											
		  //printf("\n");
          //////////////////////  中心點  /////////////////////
          ...
          //////////////////////  寬度  /////////////////////
          //取較大的寬度作為新的set的寬	
          /*if(child_sort_set_width_backup[jj][save_clu_num_inverse[jj][m]]>child_sort_set_width_backup[jj][save_clu_num_inverse[jj][n]]){  //寬度存取大的
            check_combine_data[1]=child_sort_set_width_backup[jj][save_clu_num_inverse[jj][m]];				
          }
          else{
            check_combine_data[1]=child_sort_set_width_backup[jj][save_clu_num_inverse[jj][n]];			
          }*/
          check_combine_data[1] = (child_sort_set_width_backup[jj][save_clu_num_inverse[jj][m]]+child_sort_set_width_backup[jj][save_clu_num_inverse[jj][n]])/2; 
      	  //check_combine_data[1] = child_sort_set_width_backup[jj][save_clu_num_inverse[jj][left_n]];

          child_sort_set_width_backup[jj][save_clu_num_inverse[jj][m]]=check_combine_data[1]; //中心點數值存回去					
          child_sort_set_width_backup[jj][save_clu_num_inverse[jj][n]]=check_combine_data[1]; //中心點數值存回去										
  ...
  for(int jjj=1; jjj<=_in_varl; jjj++){       
    //bb=jjj*2-1;  		
    for(int jj=1; jj<=_in_clu; jj++){	 		
      FCRule[(jj-1)*_mem_length+jjj*2-1]=child_sort_set_center[jjj][save_clu_num[jjj][jj]]; 				
      /*newpop[i].p[bb]=child_sort_set_center[jjj][save_clu_num[jjj][jj]];
      bb=bb+11;*/
    }
  }
  ...
  for(int jjj=1; jjj<=_in_varl; jjj++){  
    //bb=jjj*2;
    for(int jj=1; jj<=_in_clu; jj++){	 				
      FCRule[(jj-1)*_mem_length+jjj*2]=child_sort_set_width_backup[jjj][jj]; 					
      /*newpop[i].p[bb]=child_sort_set_center[jjj][save_clu_num[jjj][jj]];
      bb=bb+11;*/  
    }
  }
  ...
  for(int jjj=1; jjj<=_in_varl; jjj++){
    for(int jj=1; jj<=_in_clu; jj++){
      for(int jjjj=jj+1; jjjj<=_in_clu; jjjj++){
        if((FCRule[(jj-1)*_mem_length+jjj*2-1]==FCRule[(jjjj-1)*_mem_length+jjj*2-1]) && (FCRule[(jj-1)*_mem_length+jjj*2]!=FCRule[(jjjj-1)*_mem_length+jjj*2]) ){
          if (FCRule[(jj-1)*_mem_length+jjj*2]>FCRule[(jjjj-1)*_mem_length+jjj*2]){					
            FCRule[(jjjj-1)*_mem_length+jjj*2]=FCRule[(jj-1)*_mem_length+jjj*2];
          }
          else{
            FCRule[(jj-1)*_mem_length+jjj*2]=FCRule[(jjjj-1)*_mem_length+jjj*2];						
          }
        }
      }
    }
...
/////////////////////////  增加群  /////////////////////////////
/*void fuzzy_cluster()///把新增的rule轉存，後件部為隨機值
{
	int jj,jjj;
	...
	{  
		///////////////////////////////////  生成  /////////////////////////////////		
		fuzzy_real[i][(_in_clu-1)*_mem_length+jjj*2-1] = _in_v[jjj].mean[_in_clu]  ;   
		fuzzy_real[i][(_in_clu-1)*_mem_length+jjj*2] = _in_v[jjj].var[_in_clu] ;  
		///////////////////////////////////  生成  /////////////////////////////////
	}
	FCRule[i][_in_clu*_mem_length-_out_varl+1] = 1.0*(double)rand()/RAND_MAX;
	FCRule[i][_in_clu*_mem_length-_out_varl+2] = 1.0*(double)rand()/RAND_MAX;
	...
	for (jj=1; jj<=_in_clu; jj++)
	{
		for(jjj=1; jjj<=_in_varl; jjj++)
		{
			if (fuzzy_real[i][(jj-1)*_mem_length+jjj*2-1] < _center_min)
			{
				fuzzy_real[i][(jj-1)*_mem_length+jjj*2-1] = _center_min ; // 限制中心最小值
			}

			if (fuzzy_real[i][(jj-1)*_mem_length+jjj*2-1] > _center_max)
			{
				fuzzy_real[i][(jj-1)*_mem_length+jjj*2-1] = _center_max ; // 限制中心最大值
			}

			if (fuzzy_real[i][(jj-1)*_mem_length+jjj*2] < _range_min)
			{
				fuzzy_real[i][(jj-1)*_mem_length+jjj*2] = _range_min ; // 限制寬度最小值
			}

			if (fuzzy_real[i][(jj-1)*_mem_length+jjj*2] > _range_max)
			{
				fuzzy_real[i][(jj-1)*_mem_length+jjj*2] = _range_max ; // 限制寬度最大值
			}
			if(fuzzy_real[i][(jj-1)*_mem_length+jjj*2-1] < min_m[i][jjj]) ///202101 fuzzy_real[i]
			{
				min_m[i][jjj]=fuzzy_real[i][(jj-1)*_mem_length+jjj*2-1];  //如果比最大的最小值(min_m = 1)還小，就儲存最小中心點
				//cout<<"min_m["<<i<<"]["<<jjj<<"]"<<min_m[i][jjj]<<endl;
			}

			if(fuzzy_real[i][(jj-1)*_mem_length+jjj*2-1] > max_m[1][jjj]) ///202101 fuzzy_real[i]
			{
				max_m[i][jjj]=fuzzy_real[i][(jj-1)*_mem_length+jjj*2-1];  //如果比最小的最大值(max_m = 1)還大，就儲存最大中心點，
				//cout<<"max_m["<<i<<"]["<<jjj<<"]"<<max_m[i][jjj]<<endl;
			}
		}		
	}
...
void Eva_Fitness(individual* ,int, int );
void punish(individual*, int );
void checkpop(double*, int );  //單純再確認一次p式在上下界範圍內而以 //20220330
void gen_old_pop(Front*, individual* );
void mix_population(void );
void SpreadNewRules(individual ,individual *,int); //20220406
...
void turn(int num_count){
  ...
  for(int kk=0; kk<=num_count; kk++){
   ...
   //--------------------------------inorder to force realism the same amid web
   ots--------------------------------  
...
void initialization(void){//隨機產生中心點，寬，後件部，每代只用一次而已(第一次)
	...
	int maxp = 0 , current_max_rules = 0; //20220406
		...
		for(int i=1; i<=_popsize; i++){  //共40個母代，每個母代有110個參數
			...
			/*for(int j=1; j<=_rule; j++){    
				for(int k=1; k<=_in_varl; k++){    
					...
					// fuzzy sets:
					// center = random(-1~1)
					oldpop[i].p[(_gap)*(j-1)+(2*k)-1] = ((rand()%101))/100.;  //產生40個隨機中心點，且要40組(因有40隻鳥)
					//p[(_gap)*j+2*k]=p[0],p[2],p[4],p[6],p[11],p[13],p[15].p[17]...p[105]就是中心點
					// width = random(0.05~0.75)
					oldpop[i].p[(_gap)*(j-1)+(2*k)] =  0.05 + ((rand()%151)/1000.0);//產生40個隨機寬，且要40組(因有40隻鳥)
			...
			checkpop(oldpop[i].p, oldpop[i].p_rule);   // limits all parameters //20220330
    ...
    for(int i=1; i<=_popsize; i++){//計算這初始化的40組控制器的fitness，共40個的fitness
        printf("popsize=%d\n",i);
        fflush(stdout);
        Eva_Fitness(&oldpop[i],i,sw);//因Eva_Fitness輸入型別關係，所以要加&
		if(current_max_rules < oldpop[i].p_rule)
		{
			current_max_rules=oldpop[i].p_rule;
			maxp=i;
		}
		if(maxp!=0) //20220414
		{
			SpreadNewRules(oldpop[maxp], oldpop, _popsize); //20220406
			maxp = 0;
		}
   		if(check && load_gen == 1){  //20220324
		   FILE *fy2 ;
	...
	ref_rule = current_max_rules;
    for(int i=1; i<=_popsize; i++){
      checkpop(oldpop[i].p, oldpop[i].p_rule);       // limit the parameter range//20220330
      //for(int j=1; j<=1000; j++){
      check_and_combine_child_set(i, oldpop[i].p_rule, sw);     //20220406
      //}
    } 
	/*if(maxp!=0)
	{
    ...
    /////////////////////  計算當下的疊代中，最大的epsilon_sum值  /////////////////{
    int temp_num01=0;
    double ttt=0.0;
    //找最大的epsilon_sum值
    for (int ber=1; ber<=new_row; ber++){ //ber:代表第幾個解，此為一個替代的暫存變數   
      if (epsilon_sum[ber]>ttt){  	
        ttt=epsilon_sum[ber];
        temp_num01=ber;
      }	
    }
    ...
    /////////////////////////// 可解釋性的資料處理&計算 /////////////////////////////{
    double in_total = 1/double(_in_varl);
     
    for (int ber=1; ber<=_popsize; ber++){ //ber:代表第幾個解，此為一個替代的暫存變數     
      if(penality[ber]>=0 && penality[ber]<=0.01){   	
        costvalue_interpretable = epsilon_sum[ber]*in_total;
      }
      else{  	 
        costvalue_interpretable = epsilon_sum[temp_num01] + penality[ber] ;	
      } 
      oldpop[ber].CT[3] = costvalue_interpretable;
    }
    /////////////////////////// 可解釋性的資料處理&計算 /////////////////////////////}    
        
    punish(oldpop, _popsize);  // penalty function
  	//if(check){load_gen=_maxgen+1;} //20220415   
...
void generation(int iter){
    //int prt1, prt2;
    //individual parent[2];
    FILE *oldp,*all_oldp,*ofitness,*all_ofitness, *all_oldrule; //20220330
    FILE *ocd_len,*ofrnt, *oldrule; //20220330
    FILE *reference_p, *reference_rule; //20221213
    ...
    if (iter>=2){
	  ...
	  oldrule = fopen("orule.dat","r"); //20220330
	  reference_p = fopen("ref_p.dat", "r");
	  reference_rule = fopen("ref_rule.dat", "r");
      wb_robot_step(100);
      for (int a=1; a<=_popsize; a++){
        fscanf(oldrule,"%d",&oldpop[a].p_rule);     //20220330  
        for (int b=1; b<=_lpara*oldpop[a].p_rule; b++){
          fscanf(oldp,"%lf",&oldpop[a].p[b]);
        }
      ...
      fscanf(reference_rule,"%d",&ref_rule); 
	  for (int b=1; b<=_lpara*ref_rule; b++){
	    fscanf(reference_p,"%lf",&reference_FCRule[b]);
	  }
      wb_robot_step(100);
      fclose(oldp);
      fclose(ocd_len);
      fclose(ofrnt);
      fclose(ofitness);
      fclose(oldrule); //20220330
	  fclose(reference_p);
	  fclose(reference_rule);
      wb_robot_step(100);
      //********************讀oldpop用*********************
    }

	if(load_gen == 101){  //20230401 重新確認40pop的表現
		initialization();
	}

	int maxp = 0 , current_max_rules = ref_rule; //20220406    
    ...
    for(int i=1; i<=new_row; i++){
      checkpop(newpop[i].p, newpop[i].p_rule);       // limit the parameter range//20220330
      //for(int j=1; j<=1000; j++){
        check_and_combine_child_set(i, newpop[i].p_rule, sw);     //20220330
      //}
    }       
   // update_pop() ; // 解的更新
      
    for(unsigned int i=1; i<=new_row; i++){  //20220323 
        printf("現在在跑第%d代的第%d個popsize\n",iter,i);
        fflush(stdout);        
        Eva_Fitness(&newpop[i],i,sw);  // cost functions
		if(current_max_rules < newpop[i].p_rule)
		{
			current_max_rules=newpop[i].p_rule;
			maxp=i;
		}       
        ...
        fflush(stdout);
  		if(maxp!=0)  
  		{
  			SpreadNewRules(newpop[maxp], newpop, new_row); //20220414
  			SpreadNewRules(newpop[maxp], oldpop, _popsize); //20220414
  		}
		if(check){  //20220328
		   FILE *fy2 ;
		   if( (fy2=fopen("..\\..\\c_matrix.dat","a"))==NULL) exit(1) ;//存取最後成功的次數
		   ...
		   if( (fy3=fopen("..\\..\\Final_first.dat","w"))==NULL) exit(1) ;//存取最後成功的次數
                         for (int b=1; b<=_lpara*newpop[i].p_rule; b++){ //20220330
                            fprintf(fy3,"%lf\n",newpop[i].p[b]); //20220328
                          }
		   fclose(fy3) ;

		   FILE *fy4 ;
		   if( (fy4=fopen("..\\..\\Final_clu.dat","w"))==NULL) exit(1) ;//存取最後成功的次數
                            fprintf(fy4,"%d\n",newpop[i].p_rule); //20220328
		   fclose(fy4) ;
		   check = 0; //20220415
		   //break;
		}         
    ...
    //if(check){ //20220407
      for(int i=1; i<=new_row; i++){
        checkpop(newpop[i].p, newpop[i].p_rule);       // limit the parameter range//20220330
        //for(int j=1; j<=1000; j++){
        check_and_combine_child_set(i, newpop[i].p_rule, sw);     //20220406
        //}
      } 
  	/*if(maxp!=0)
  	{
  	    SpreadNewRules(newpop[maxp], newpop, new_row); //20220406
  		SpreadNewRules(newpop[maxp], oldpop, _popsize); //20220406
  	}*/
    //}
    /////////////////////  計算當下的疊代中，最大的epsilon_sum值  /////////////////{
    int temp_num01=0;
    ...
    for (int ber=1; ber<=new_row; ber++){ //ber:代表第幾個解，此為一個替代的暫存變數     //20220328
      if(penality[ber]>=0 && penality[ber]<=0.01){   	
        costvalue_interpretable = epsilon_sum[ber]*in_total;
        //printf("costvalue_interpretable=%lf\n",costvalue_interpretable);
      } 
      else{     	 
        costvalue_interpretable = epsilon_sum[temp_num01] + penality[ber] ;
        printf("costvalue_interpretable=%lf\n",costvalue_interpretable);
      } 
      newpop[ber].CT[3] = costvalue_interpretable;	
      //printf("newpop[%d].CT[3]=%lf\n",ber,newpop[ber].CT[3]);	
    }
    
    /////////////////////////// 可解釋性的資料處理&計算 /////////////////////////////}

    punish(newpop, new_row);   // penalty functions //20220328
 	//if(check){load_gen=_maxgen+1;} //20220415   
    /*for (int i=1 ; i<=_popsize ; i++){
	...
	all_oldrule = fopen("all_orule.dat","a"); //20220330
    ocd_len = fopen("ocd_len.dat","w");
    ofrnt = fopen("ofrnt.dat","w");
    ofitness = fopen("ofitness.dat","w");
	oldrule = fopen("orule.dat","w"); //20220330
	reference_p = fopen("ref_p.dat", "w");
	reference_rule = fopen("ref_rule.dat", "w");
    fprintf(all_oldp,"*************以下是第%d代的參數值*********\n",iter);
    fprintf(all_ofitness,"*************以下是第%d代的參數值*********\n",iter);
	fprintf(all_oldrule,"*************以下是第%d代的參數值*********\n",iter);//20220330

    for (int a=1; a <= _popsize; a++){
      fprintf(oldrule,"%d\n",oldpop[a].p_rule);   //20220330
      fprintf(all_oldrule,"%d\t",oldpop[a].p_rule);//20220330
	  for (int b=1; b <= _lpara*oldpop[a].p_rule; b++){ //20220330
        fprintf(oldp,"%lf\n",oldpop[a].p[b]);
    ...
    fprintf(reference_rule,"%d",ref_rule); 
	for (int b=1; b<=_lpara*ref_rule; b++){
		fprintf(reference_p,"%lf\n",reference_FCRule[b]);
	}
    ...
    if(iter>=_maxgen){ //20220414
    
      save_final_pareto_front() ;
...
void Eva_Fitness(individual* pop, int x,int s){
    int step=200, robot_state=0, DD=0;
    double c1, c2, c3;
    double ct1=0, ct2=0, ct3=0, ct4=0, dw=0.2;
...
//********************************************testing*****************************************************  
        ...
        if(leg[1].osc[1].Y[step-1]<=0 && leg[1].osc[1].Y[step]>0){
		  ...
		  fc.FC_Str_Learn(in, load_gen, &(pop->p_rule)); //20220330    
		  fc.cal_rule = pop->p_rule; //20220330
		  fc.check_and_merge_in_train(pop->p_rule); //20220402
...
//******************************************testing*******************************************************     
        ...
        */
        ...
        // ct1: sum( (dis(t)/0.5) - 1)
        //c1= fabs(((Urg[0]-0.3)/dw)-1);
        c1= fabs(((Urg[0]-0.2)/dw)-1); //20220328
		//控制沿牆距離
    ...
    robot_get_position(pos); 
    wb_robot_step(TIME_STEP);
	for(int i=1; i<=_lpara*(pop->p_rule); i++){  //20220330
		pop->p[i] = fc.FCRule[i];
	}
    overlap_degree(x, s, pop->p_rule);  //重疊度的計算 //20220330
    fuzzy_set_constraint(x, s, pop->p_rule);  //fuzzy set之間的距離限制//20220330
...
void punish(individual* eva, int count){
    for (int i=1;i<=count;i++){
        ...
        eva[i].fitness[3]=eva[i].CT[0]+eva[i].CT[3];  
		//////////////////20220328////////////////////
		FILE *fr;
		if( (fr=fopen("..\\..\\avg_reward.dat","a"))==NULL) exit(1) ; 
		fprintf(fr,"%lf\t%lf\t%lf\t", eva[i].fitness[1], eva[i].fitness[2], eva[i].fitness[3]); 
		fprintf(fr,"\n");
		fclose(fr) ;
		////////////////////////////////////////////
    }
...
////////////////////////////////////////////////////  fuzzy set的合併  /////////////////////////////////////////////////{
void check_and_combine_child_set(int i, int _in_clu, int sw){ //20220406
////////////// 檢查合併的暫存變數 //////////////
  double check_dis_variable=0.0;
  double check_combine_data[3]={0.};
  double bigger_width=0.0;
  ...
  if(sw == 1){
	for(int a=1; a<=_in_clu; a++){  //jj表示第幾條rule 
		for(int b=1; b<=_in_varl; b++){ //jjj表示第幾個輸入
		  ...
		  child_sort_set_center[b][a] = oldpop[i].p[((a-1)*_mem_length)+((b*2)-1)] ; // 中心點
		  //printf("child_sort_set_center[%d][%d]=%lf\n",b,a,child_sort_set_center[b][a]);
		  child_sort_set_center_backup[b][a] = oldpop[i].p[(a-1)*_mem_length+b*2-1] ; // 中心點    
		  child_sort_set_width_backup[b][a] = oldpop[i].p[(a-1)*_mem_length+b*2] ; // 寬度   
  ...
  //int bb=1;
	  ...
	  for(int a=1; a<=_in_clu; a++){  //jj表示第幾條rule 
		for(int b=1; b<=_in_varl; b++){ //jjj表示第幾個輸入
		  ...
		  child_sort_set_center[b][a] = newpop[i].p[((a-1)*_mem_length)+((b*2)-1)] ; // 中心點
		  //printf("child_sort_set_center[%d][%d]=%lf\n",b,a,child_sort_set_center[b][a]);
		  child_sort_set_center_backup[b][a] = newpop[i].p[(a-1)*_mem_length+b*2-1] ; // 中心點    
		  child_sort_set_width_backup[b][a] = newpop[i].p[(a-1)*_mem_length+b*2] ; // 寬度   
...
////////////////////////  利用插入排序法排列中心點的大小順序  ////////////////////////
  ...
  //bb=1;
	  for(int jjj=1; jjj<=_in_varl; jjj++){       
		//bb=jjj*2-1;  		
		for(int jj=1; jj<=_in_clu; jj++){	 		
		  oldpop[i].p[(jj-1)*_mem_length+jjj*2-1]=child_sort_set_center[jjj][save_clu_num[jjj][jj]]; 				
		  /*oldpop[i].p[bb]=child_sort_set_center[jjj][save_clu_num[jjj][jj]];
		  bb=bb+11;*/
		}
	  }
	  ...
	  for(int jjj=1; jjj<=_in_varl; jjj++){  
		//bb=jjj*2;
		for(int jj=1; jj<=_in_clu; jj++){	 				
		  oldpop[i].p[(jj-1)*_mem_length+jjj*2]=child_sort_set_width_backup[jjj][jj]; 					
		  /*oldpop[i].p[bb]=child_sort_set_center[jjj][save_clu_num[jjj][jj]];
		  bb=bb+11;*/  
		}
	  }
	  ...
	  for(int jjj=1; jjj<=_in_varl; jjj++){
		for(int jj=1; jj<=_in_clu; jj++){
		  for(int jjjj=jj+1; jjjj<=_in_clu; jjjj++){
			if((oldpop[i].p[(jj-1)*_mem_length+jjj*2-1]==oldpop[i].p[(jjjj-1)*_mem_length+jjj*2-1]) && (oldpop[i].p[(jj-1)*_mem_length+jjj*2]!=newpop[i].p[(jjjj-1)*_mem_length+jjj*2]) ){
			  if (oldpop[i].p[(jj-1)*_mem_length+jjj*2]>oldpop[i].p[(jjjj-1)*_mem_length+jjj*2]){					
				oldpop[i].p[(jjjj-1)*_mem_length+jjj*2]=oldpop[i].p[(jj-1)*_mem_length+jjj*2];
			  }
			  else{
				oldpop[i].p[(jj-1)*_mem_length+jjj*2]=oldpop[i].p[(jjjj-1)*_mem_length+jjj*2];						
			  }
			}
		  }
		}
	  ...
	  for(int jjj=1; jjj<=_in_varl; jjj++){       
		//bb=jjj*2-1;  		
		for(int jj=1; jj<=_in_clu; jj++){	 		
		  newpop[i].p[(jj-1)*_mem_length+jjj*2-1]=child_sort_set_center[jjj][save_clu_num[jjj][jj]]; 				
		  /*newpop[i].p[bb]=child_sort_set_center[jjj][save_clu_num[jjj][jj]];
		  bb=bb+11;*/
		}
	  }
	  ...
	  for(int jjj=1; jjj<=_in_varl; jjj++){  
		//bb=jjj*2;
		for(int jj=1; jj<=_in_clu; jj++){	 				
		  newpop[i].p[(jj-1)*_mem_length+jjj*2]=child_sort_set_width_backup[jjj][jj]; 					
		  /*newpop[i].p[bb]=child_sort_set_center[jjj][save_clu_num[jjj][jj]];
		  bb=bb+11;*/  
		}
	  }
	  ...
	  for(int jjj=1; jjj<=_in_varl; jjj++){
		for(int jj=1; jj<=_in_clu; jj++){
		  for(int jjjj=jj+1; jjjj<=_in_clu; jjjj++){
			if((newpop[i].p[(jj-1)*_mem_length+jjj*2-1]==newpop[i].p[(jjjj-1)*_mem_length+jjj*2-1]) && (newpop[i].p[(jj-1)*_mem_length+jjj*2]!=newpop[i].p[(jjjj-1)*_mem_length+jjj*2]) ){
			  if (newpop[i].p[(jj-1)*_mem_length+jjj*2]>newpop[i].p[(jjjj-1)*_mem_length+jjj*2]){					
				newpop[i].p[(jjjj-1)*_mem_length+jjj*2]=newpop[i].p[(jj-1)*_mem_length+jjj*2];
			  }
			  else{
				newpop[i].p[(jj-1)*_mem_length+jjj*2]=newpop[i].p[(jjjj-1)*_mem_length+jjj*2];						
			  }
			}
		  }
		}
...
//////////////////////////////////////////  重疊度的計算  /////////////////////////////////////////{
void overlap_degree(int ber, int s,int rule){
  float k;
  ...
  if(s==1){
    for(int i=1;i<=_lpara*rule;i++){ //20220330
      fuzzy_real_compare[i]=oldpop[ber].p[i]; 
    }
  }
  else if(s==3){
    for(int i=1;i<=_lpara*rule;i++){ //20220330
      fuzzy_real_compare[i]=newpop[ber].p[i]; 
    }
  }  
  //排除重複的規則
  for(int jjj=1; jjj<=_in_varl; jjj++){
    
    for(int jj=1; jj<=rule; jj++){
    
      for(int jjjj=jj+1; jjjj<=rule; jjjj++){
  ...
  for(int jjj=1; jjj<=_in_varl; jjj++){
	...
	/////////////////////  間隔內的激發量和  ///////////////////{
	n=Gaussian(k, fuzzy_real_compare[(jj-1)*_mem_length+jjj*2-1], fuzzy_real_compare[(jj-1)*_mem_length+jjj*2]);							 	 			
	mr=mr+n;
	/////////////////////  間隔內的激發量和  ///////////////////}
      }	
  ...
  printf("epsilon_sum[%d]=%lf\n",ber,epsilon_sum[ber]);
}
//////////////////////////////////////////  重疊度的計算  /////////////////////////////////////////} 


//////////////////////////////////  fuzzy set之間的距離限制  /////////////////////////////////{
void fuzzy_set_constraint(int ber, int s, int rule){
	
//目的:限制set之間中心點的距離不要過小

  double dis=0.,delta_1,g=0.,max=0.;
  double delta_2=0, h=0.;
  ...
  for(int jjj=1; jjj<=_in_varl; jjj++){		
    for(int jj=1;jj<=rule;jj++){
			if(fuzzy_real_compare[(jj-1)*_mem_length+jjj*2]<_range_min)
			{
				delta_2=(fuzzy_real_compare[(jj-1)*_mem_length+jjj*2]-_range_min)*(fuzzy_real_compare[(jj-1)*_mem_length+jjj*2]-_range_min);
			}
		    else if(fuzzy_real_compare[(jj-1)*_mem_length+jjj*2]>_range_max)
			{
				delta_2=(fuzzy_real_compare[(jj-1)*_mem_length+jjj*2]-_range_max)*(fuzzy_real_compare[(jj-1)*_mem_length+jjj*2]-_range_max);
			}
			else if(fuzzy_real_compare[(jj-1)*_mem_length+jjj*2]<=_range_max && fuzzy_real_compare[(jj-1)*_mem_length+jjj*2]>=_range_min )
			{
				delta_2=0.0;
			}
			h=delta_2+h;
  ...
  for(int jj=1; jj<=_in_varl; jj++){		
    for(int m=1;m<=rule;m++){
      savep=1.0;	
      for(int n=1;n<=rule;n++){		
      //兩個被挑選出來的中心點距離差		
...
void mutation()
{
  ...
  {
    for(j=1; j<=_lpara*newpop[i].p_rule; j++) //20220330
    {
    rnd=randn(0,1);
...
rnd=randn(0,1);
  ...
  if(rnd<=_pc)
  {
    for(i=1; i<=_lpara*oldpop[_tour1].p_rule; i++)
    {
...
void update_newparent()
{
	...
	for(i=1; i<=_popsize+new_row; i++)
	{
		...
		for (j=1; j<=_lpara*_max_rule; j++) //20220330
			newpop[i].p[j] = 0 ;
...
void checkpop(double* p, int rule){  //單純再確認一次p式在上下界範圍內而以 //20220330
    double pmax, pmin;
...
int robot_state_judge(double *dis){
	...
	if( (dis[0]<0.25) || (dis[1]<0.3) || (dis[2]<0.3) || (dis[3]<0.3)){ //20220328
...
void save_final_obj(int j){
  ...
  if (j==1){
    ...
    if( (f_obj=fopen("w.txt","a"))==NULL) exit(1) ;
    {
      for(jj=1; jj<=oldpop[best_solution[j]].p_rule; jj++){ //20220414
        for(jjj=1; jjj<=_in_varl; jjj++){
          fprintf(f_obj,"%lf\t", oldpop[best_solution[j]].p[(jj-1)*_mem_length+jjj*2-1] ) ; 
          fprintf(f_obj,"%lf\t", oldpop[best_solution[j]].p[(jj-1)*_mem_length+jjj*2] ) ; 	  
        }
        fprintf(f_obj,"%lf   \t",  oldpop[best_solution[j]].p[jj*_mem_length-_out_varl+1] ) ; 
        fprintf(f_obj,"%lf   \t",  oldpop[best_solution[j]].p[jj*_mem_length-_out_varl+2] ) ;    
        //fprintf(f_obj,"%lf   \t",  oldpop[best_solution[j]].p[jj*_mem_length-_out_varl+3] ) ;   
      }
    }
    fprintf(f_obj,"\n") ;
    fclose(f_obj) ;
  ...
  if (j==1){	
    ...
    if( (f_obj=fopen("cost_w.txt","a"))==NULL) exit(1) ;
    {
       for(jj=1; jj<=_nobj;jj++)  
       {
           fprintf(f_obj,"%lf\t", oldpop[best_solution[j]].fitness[jj]) ;  //cost value
       }
    }
  fprintf(f_obj,"\n") ;
  fclose(f_obj) ;
...
//--------------------------------------------------------------------------------------------------------  
  if(j==2){
    ...
    if( (f_obj=fopen("s.txt","a"))==NULL) exit(1) ;
    {
      for(jj=1; jj<=oldpop[best_solution[j]].p_rule; jj++) //20220414
      { 
        for(jjj=1; jjj<=_in_varl; jjj++)
  ...
  if (j==2){	
    ...
    if( (f_obj=fopen("cost_s.txt","a"))==NULL) exit(1) ;
    {
       for(jj=1; jj<=_nobj;jj++)  
       {
           fprintf(f_obj,"%lf\t", oldpop[best_solution[j]].fitness[jj]) ;  //cost value
...
//--------------------------------------------------------------------------------------------------------
  if(j==3){
    ...
    if( (f_obj=fopen("i.txt","w"))==NULL) exit(1) ;
    {
      for(jj=1; jj<=oldpop[best_solution[j]].p_rule; jj++)//20220414
      { 
        for(jjj=1; jjj<=_in_varl; jjj++){
  ...
  if (j==3){	
    ...
    if( (f_obj=fopen("cost_i.txt","w"))==NULL) exit(1) ;
    {
       for(jj=1; jj<=_nobj;jj++){
         fprintf(f_obj,"%lf\t", oldpop[best_solution[j]].fitness[jj]) ;  //cost value
       }
...
void save_final_pareto_front(){
  ...
  int i,j ;
  FILE *fpt_init;	 
  fpt_init = fopen("final_pareto_front00.txt","a") ;

  for (i=1; i<=_popsize; i++){  
    for(j=1; j<=_nobj;j++){
      fprintf(fpt_init,"%lf\t", oldpop[i].fitness[j]) ;  //cost value
    }
    fprintf(fpt_init,"%d\r\n", oldpop[i].frnt); //排名	
  } 
  ...
  FILE *fpt_init01;
  fpt_init01 = fopen("final_pareto_front.txt","a") ; 

  for (i=1; i<=_popsize; i++){
    for(j=1; j<=_nobj;j++){
      fprintf(fpt_init01,"%lf\t", oldpop[i].fitness[j]) ;  //cost value		
    }
    fprintf(fpt_init01, "\r\n") ;
  }
  fclose(fpt_init01) ;
...
void report_best(){
 ...
 double sorted_temp[2*_popsize+1][_nobj+1] ;
 int i,pass ;  
  
 for (int j=1; j<=_nobj; j++){
 
   int w = 1 ;	   
   for (i=1; i<=_popsize;i++){ 
     sorted_temp[i][j] = oldpop[i].fitness[j] ;  //轉存 cost value 
   }  
  
   for (pass=2; pass<=_popsize-1; pass++){
...
void update_ant(int t_loop){
  ...
  for (int kk=1; kk<=t_loop; kk++){//t_loop為L個
    ...
    _tour1 = kk ;   
    ////////////  phase1:菁英解(1~L)  ///////////}
	rule = oldpop[ _tour1 ].p_rule; //20220330
	newpop[kk].p_rule = rule;  //20220330
    for (ww=1; ww<=_lpara*rule; ww++){ //20220330
      aa = 0. ;
    ...
    ////////////////////////  向最佳解趨近  //////////////////////{
    if(oldpop[l_best].p_rule == rule){ //20220330
		for(ww=1; ww<=_lpara*rule; ww++){
...
////////////////////////////////////////////////////////////////////////
/////////////////////////20220406/////////////////////////
void SpreadNewRules(individual evapop, individual *new_pop, int spreadsize)
{
	int pop, i, j, k; 
	int nr, n=(2*_in_varl+_out_varl);

	for(pop=1; pop<=spreadsize; pop++)
	{
		nr=new_pop[pop].p_rule;
		
		if( evapop.p_rule>nr )
		{
			for(i=nr+1; i<=evapop.p_rule; i++)
			{
				for(j=1; j<=_in_varl; j++)
				{
					new_pop[pop].p[(i-1)*n+2*j-1]=reference_FCRule[(i-1)*n+2*j-1]+randn(-0.1,0.1);
					new_pop[pop].p[(i-1)*n+2*j]=reference_FCRule[(i-1)*n+2*j]+randn(-0.1,0.1);
				}

				for(k=1; k<=_out_varl; k++)
				{
					new_pop[pop].p[(i-1)*n+2*_in_varl+k]=randn(_conq_min, _conq_max);
				}
			   
			}// loop over i ends
			
			//new_pop[pop].checkpop();
			new_pop[pop].p_rule=evapop.p_rule;
			checkpop(new_pop[pop].p, new_pop[pop].p_rule);   // limits all parameters //20220330
			if(spreadsize == 40)
			{
				//CombineIndRules(new_pop[p].p, new_pop[p].p_rule);
				check_and_combine_child_set(pop, new_pop[pop].p_rule, 1);     //20220406
			}
			else if(spreadsize == 20)
			{
				//CombineIndRules(new_pop[p].p, new_pop[p].p_rule);
				check_and_combine_child_set(pop, new_pop[pop].p_rule, 3);     //20220406
			}
		}
	}
	//_getch();
