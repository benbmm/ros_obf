  ...
  void cal_out_3_walk(int num_count) {
    ...
    for (int i = 0; i < NUM_SERVOS; i = i + 3) {
      ...
      if (i == 0 || i == 3 || i == 6) {
        ...
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "leg[%d].osc[2].Y[%d]=%f,173",
                    j, num_count, leg[j].osc[2].Y[num_count]);
        if (leg[j].osc[2].Y[num_count] < 0) {
          ...
          if (leg[j].ww ==
            ...
            if (leg[j].no >= 10) {
              RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "no=%d,adjust success",
                          leg[j].no);
              judge[j] = 1;  // adjust success
              leg[j].no = 0;
              leg[j].ww = 2;
            } else {
              RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "no=%d,adjust failed",
                          leg[j].no);
        ...
        //------------------------------------------------------------------------------------
          ...
          pi[j] = pitch;
          ro[j] = roll;
          RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "pitch=%f,roll=%f,373",
                      pitch, roll);
          if (fabs(pi[j]) < thres &&
      ...
      if (i == 9 || i == 12 || i == 15) {
        ...
        //------------------------------------------------------------------------------------
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "leg[%d].osc[2].Y[%d]=%f,400",
                    j, num_count, leg[j].osc[2].Y[num_count]);
        if (leg[j].osc[2].Y[num_count] > 0) {
          RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "ww=%d", leg[j].ww);
          if (leg[j].ww == 1) {
        ...
        //------------------------------------------------------------------------------------
          ...
          pi[j] = pitch;
          ro[j] = roll;
          RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "pitch=%f,roll=%f,602",
                      pitch, roll);
